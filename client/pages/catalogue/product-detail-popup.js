  ...
  render() {
    return html`
      <wizard-summary>
        ...
        <p>3 easy steps to create your new product and sync to marketplace seamlessly</p>
        <ol>
          <li ?active=${this.current == 'basic-product-setting'}>Product Basic Information</li>
          <li ?active=${this.current == 'product-variation-setting'}>Variation Information</li>
          <li ?active=${this.current == 'product-marketplace-setting'}>Marketplace Settings</li>
        </ol>
      </wizard-summary>
      <wizard-view @change=${e => (this.current = e.detail.active?.getAttribute('name'))} @done=${e => this.done(e)}>
        <basic-product-setting
          name="basic-product-setting"
          .productInfo=${this.productInformation}
          @submit="${e => (this.productInfo = e.marketplaceProduct)}"
        ></basic-product-setting>
        <product-variation-setting
          name="product-variation-setting"
          @submit="${e => ((this.productInfo = e.marketplaceProduct), (this.variationInfo = e.variationInfo))}"
        ></product-variation-setting>
