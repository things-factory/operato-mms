  ...
  render() {
    return html`
      ...
      <div class="button-container">
        <button @click=${this._editProduct.bind(this)}>${i18next.t('button.update')}</button>
        <button @click=${this._deleteProduct.bind(this)}>${i18next.t('button.delete')}</button>
      </div>
    `
  }
  ...
  async _updateProduct() {
    try {
      ...
      const response = await client.query({
        query: gql`
            mutation {
              updateMarketplaceProduct(${gqlBuilder.buildArgs({
                marketplaceProduct
              })}){
                name
                description
  ...
  async _deleteProduct() {
    try {
      const response = await client.query({
        query: gql`
            mutation {
              deleteMarketplaceProduct(${gqlBuilder.buildArgs({
                marketplaceProduct
              })}){
                name
                description
